...
b32 g2_button(buf label, v2 coords) {
  ...
  rect r = bfont_calc_text_rect(ctx.font, label, coords, ctx.g_scale);
  ...
  bfont_draw_text(ctx.font, ctx.temp_arena, ctx.viewport , ctx.viewport, label, v2m(coords.x, coords.y), ctx.g_scale, col(color_mod, color_mod, color_mod,1), true);
  return res;
}
